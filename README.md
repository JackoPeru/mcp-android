...
La v0.8.12 mantiene i **due trasporti indipendenti** della v0.8.2 e porta l'updater Android allo stesso flusso esplicito usato da HermesHub:
...
APK disponibile: **`dist/mcp-android-0.8.12-debug.apk`**, con SHA-256 nel file accanto. È una build debug firmata per installazione personale, non una release Play Store.
...
- Nessun aggiramento PIN, biometria, blocco schermo o finestre protette — tranne `android_unlock_device`, che usa solo il PIN che hai salvato cifrato in-app (chi possiede il token può chiederne lo sblocco). Contenuti sensibili possono essere nascosti dal sistema; alcune app espongono poche informazioni Accessibilità.
...
La v0.8.12 conserva la modalità ultra-low-power della v0.7.2 e il dual transport event-driven:
