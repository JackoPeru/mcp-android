...
- Termux >= 0.109 da F-Droid (la versione Play Store è obsoleta) solo se si vuole usare android_shell; il permesso Run commands in Termux environment resta separato e deve essere concesso dall'utente. Su OxygenOS aggressivi, togli MCP Android dall'ottimizzazione batteria se i comandi falliscono all'avvio.
...
La v0.8.5 mantiene i **due trasporti indipendenti** della v0.8.2 e porta l'updater Android allo stesso flusso esplicito usato da HermesHub:
...
APK disponibile: **`dist/mcp-android-0.8.5-debug.apk`**, con SHA-256 nel file accanto. È una build debug firmata per installazione personale, non una release Play Store.
...
La v0.8.5 conserva la modalità ultra-low-power della v0.7.2 e il dual transport event-driven:
