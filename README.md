...
La v0.8.15 mantiene i **due trasporti indipendenti** della v0.8.2 e porta l'updater Android allo stesso flusso esplicito usato da HermesHub:
...
APK disponibile: **`dist/mcp-android-0.8.15-debug.apk`**, con SHA-256 nel file accanto. È una build debug firmata per installazione personale, non una release Play Store.
...
| `android_unlock_device` | Sveglia lo schermo e sblocca col PIN salvato in-app (cifrato, mai trasmesso). Un tentativo verificato; chi ha il token può chiederlo |
...
- Nessun aggiramento PIN, biometria, blocco schermo o finestre protette — tranne `android_unlock_device`, che usa solo il PIN che hai salvato cifrato in-app (chi possiede il token può chiederne lo sblocco). A schermo bloccato, nelle Impostazioni scegli tu: accesso completo (tutto tranne la UI, predefinito) o solo lettura notifiche più sblocco. Contenuti sensibili possono essere nascosti dal sistema; alcune app espongono poche informazioni Accessibilità.
...
La v0.8.15 conserva la modalità ultra-low-power della v0.7.2 e il dual transport event-driven:
