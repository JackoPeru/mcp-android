...
import { discoverLan } from '../bridge/discovery.js';
...
test('one-shot discovery sends no token and validates nonce before accepting candidate', async t => {
  ...
  assert.deepEqual(candidates, ['http://192.168.1.84:8765/']);
...
test('discovery timeout is bounded and malformed responses are ignored', async () => {
  ...
  const candidates = await discoverLan({ timeoutMs: 60, port: 65500, targets: [{ broadcast: '127.0.0.1' }] });
